...
LoopClosing::LoopClosing(Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale, const bool bActiveLC):
    ...
    mpSystem(nullptr), mbForceMergeAfterLoad(false)
{
...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        {
            // ------------------- 强制合并zyq -------------------
            bool mbMergeDetectedzyq = false;
            if(mbForceMergeAfterLoad)
            {
                cout << "!!! Force Merge Activated !!!" << endl;
                ...
                // 1. 使用现有函数从数据库中寻找最佳候选帧
                vector<KeyFrame*> vpMergeCandidates, vpLoopCandidates;
                mpKeyFrameDB->DetectNBestCandidates(mpCurrentKF, vpLoopCandidates, vpMergeCandidates, 1); // 只找1个最佳候选

                if(!vpMergeCandidates.empty())
                {
                    KeyFrame* pCandidateKF = vpMergeCandidates[0];
                    cout << "Found a candidate KF " << pCandidateKF->mnId << " from loaded map to force merge with KF " << mpCurrentKF->mnId << "." << endl;

                    // 2. 设置合并所需的成员变量
                    mpMergeMatchedKF = pCandidateKF;
                    mpMergeLastCurrentKF = mpCurrentKF;
                    mpMergeLastCurrentKF->SetNotErase();
                    mpMergeMatchedKF->SetNotErase();

                    // 3. 伪造一个单位变换 (Sim3)，假设两个KF位姿相同
                    // 这是最关键的简化，我们不进行复杂的几何验证，直接假设它们匹配
                    Sophus::SE3d T_identity; // 默认是单位矩阵
                    mg2oMergeSlw = g2o::Sim3(T_identity.unit_quaternion(), T_identity.translation(), 1.0);
                    mMergeCameraPair = CameraMatch(0,0); // 假设使用主相机

                    // 4. 强制设置合并标志
                    mbMergeDetected = true;
                    mbMergeDetectedzyq = true;
                }
                else
                {
                    cout << "!!! Force Merge Failed: No candidate KF found in database. Re-inserting KF." << endl;
                    // 如果失败，把当前KF放回队列，下次再试
                    unique_lock<mutex> lock2(mMutexLoopQueue);
                    mlpLoopKeyFrameQueue.push_front(mpCurrentKF);
                    lock2.unlock();
                }
            }
            // ----------------------------------------------------------
            ...
            if(!mbMergeDetectedzyq)
                bFindedRegion = NewDetectCommonRegions();

            // 检查新来的KF是否请求全局BA
            if (mpCurrentKF->mbForceGlobalBA) {
                Map* pCurrentMap = mpAtlas->GetCurrentMap();
                if(pCurrentMap && pCurrentMap->KeyFramesInMap() > 50) {
                    cout << "[TagLC] Starting Global BA due to Tag re-observation request from KF " << mpCurrentKF->mnId << endl;
                    ...
                    // 模仿LoopCorrect
                    // 一、计算位姿校正量并传播
                    // 获取Tag的世界位姿真值 (T_w_tag)
                    Eigen::Matrix3d R_w_tag;
                    Eigen::Vector3d t_w_tag;
                    double zyq;
                    if (TagStorage::Instance().tagRead(mpCurrentKF->mTagGBAId, R_w_tag, t_w_tag)) {
                        TagStorage::Instance().tagRead4LC(mpCurrentKF->mTagGBAId, R_w_tag, t_w_tag, zyq);
                        Sophus::SE3f T_w_tag(R_w_tag.cast<float>(), t_w_tag.cast<float>());

                        // 计算当前帧基于Tag的"正确"世界位姿 (T_w_c_correct)
                        Sophus::SE3f T_w_c_correct = T_w_tag * mpCurrentKF->mTagGBAPose_cam_tag.inverse();
                        // cout << "正确" << T_w_c_correct.translation() << endl;

                        // 获取当前帧的"错误"世界位姿 (T_w_c_wrong)
                        Sophus::SE3f T_w_c_wrong = mpCurrentKF->GetPoseInverse();
                        // cout << "错误" << T_w_c_wrong.translation() << endl;

                        cout << "差异" << (T_w_c_correct.translation() - T_w_c_wrong.translation()).norm() << endl;

                        // 获取当前KF的共视帧，并将校正传播给它们
                        vector<KeyFrame*> vpLocalKFs = mpCurrentKF->GetVectorCovisibleKeyFrames();
                        //vector<KeyFrame*> vpLocalKFs = mpCurrentKF->GetBestCovisibilityKeyFrames(30);
                        vpLocalKFs.push_back(mpCurrentKF);
                        
                        // 提前备份校正前的位姿
                        KeyFrameAndPose NonCorrectedSim3;
                        for(KeyFrame* pKFi : vpLocalKFs) {
                            if(pKFi->isBad()) continue;
                            Sophus::SE3f Tcw_wrong = pKFi->GetPose();
                            g2o::Sim3 g2oScw_wrong(Tcw_wrong.unit_quaternion().cast<double>(), Tcw_wrong.translation().cast<double>(), 1.0);
                            NonCorrectedSim3[pKFi] = g2oScw_wrong;
                        }
                        ...
                        for(KeyFrame* pKFi : vpLocalKFs) {
                            if(pKFi->isBad()) continue;
                            // 限制传播范围
                            if(pKFi->mnId < 50) continue; // 运行初期不传播
                            // 限制只传播较近的共视关键帧
                            if(pKFi->mnId < mpCurrentKF->mnId / 2) continue;
                            if((pKFi->mnId < mpCurrentKF->mnId * 2 / 3) && (mpCurrentKF->mnId > 180)) continue;
                            if(pKFi->mnId < mpCurrentKF->mnId - 60) continue;

                            // 更新位姿，利用相对位置进行传播
                            Sophus::SE3f T_w_i_wrong = pKFi->GetPoseInverse(); //Twc
                            Sophus::SE3f T_w_i_correct = T_w_c_correct * (T_w_c_wrong.inverse() * T_w_i_wrong);
                            pKFi->SetPose(T_w_i_correct.inverse()); //Tcw

                            // 更新地图点
                            vector<MapPoint*> vpMPs = pKFi->GetMapPointMatches();
                            for(MapPoint* pMP : vpMPs) {
                                if(pMP == nullptr || pMP->isBad() || pMP->mnCorrectedByKF == mpCurrentKF->mnId)
                                    continue;
                                
                                // 只校正那些以当前pKFi为参考帧的地图点，避免重复校正
                                if(pMP->GetReferenceKeyFrame() == pKFi)
                                {
                                    Eigen::Vector3f P_w_wrong = pMP->GetWorldPos();
                                    // 将点从错误世界坐标转换到pKFi相机坐标，再通过pKFi的正确位姿转换回世界
                                    Eigen::Vector3f P_w_correct = T_w_i_correct.cast<float>() * (T_w_i_wrong.inverse().cast<float>() * P_w_wrong);
                                    pMP->SetWorldPos(P_w_correct);
                                    pMP->mnCorrectedByKF = mpCurrentKF->mnId; // 标记以避免被其他帧重复校正
                                }
                            }
                        }
                        }
                        // 二、地图点融合
                        // 准备当前侧的已校正位姿 (构建 CorrectedSim3 参数)
                        KeyFrameAndPose CorrectedPosesMap;
                        for(KeyFrame* pKFi : vpLocalKFs) {
                            if(pKFi->isBad()) continue;
                            // 将校正后的 SE3 位姿转换为 g2o::Sim3 (尺度为1)
                            g2o::Sim3 g2oSic(pKFi->GetPose().unit_quaternion().cast<double>(), pKFi->GetPose().translation().cast<double>(), 1.0);
                            CorrectedPosesMap[pKFi] = g2oSic;
                        }

                        // 准备历史侧的地图点候选池 (构建 mvpLoopMatchedMPs 参数)
                        vector<MapPoint*> vpTargetMPs;
                        set<MapPoint*> spTargetMPs; // 使用set来防止重复添加

                        // a. 从TagStorage中获取所有观测到该Tag的历史关键帧
                        vector<KeyFrame*> vpHistoricalKFs = TagStorage::Instance().GetObservingKFForTag(mpCurrentKF->mTagGBAId);

                        cout << "[TagLC] Found " << vpHistoricalKFs.size() << " historical KFs observing Tag " << mpCurrentKF->mTagGBAId << "." << endl;

                        set<KeyFrame*> spHistoricalNeighborhood; // 历史邻域的集合，防止重复处理
                        for(KeyFrame* pHistKF : vpHistoricalKFs) {
                            if(!pHistKF || pHistKF->isBad() || pHistKF == mpCurrentKF) continue;

                            // b. 将历史关键帧本身加入邻域
                            spHistoricalNeighborhood.insert(pHistKF);
                            // c. 将历史关键帧的共视邻居也加入邻域
                            vector<KeyFrame*> vpCovisibles = pHistKF->GetBestCovisibilityKeyFrames(10); // 获取10个最佳共视邻居
                            spHistoricalNeighborhood.insert(vpCovisibles.begin(), vpCovisibles.end());
                        }

                        cout << "[TagLC] Total historical neighborhood size: " << spHistoricalNeighborhood.size() << " KFs." << endl;

                        // d. 从整个历史邻域中收集所有地图点
                        for(KeyFrame* pKFi_hist : spHistoricalNeighborhood) {
                            if(!pKFi_hist || pKFi_hist->isBad()) continue;

                            vector<MapPoint*> vpMPs_hist = pKFi_hist->GetMapPointMatches();
                            for(MapPoint* pMP : vpMPs_hist) {
                                if(!pMP || pMP->isBad()) continue;
                                // 只有当这个点没被添加过时，才加入
                                if(spTargetMPs.insert(pMP).second) {
                                    vpTargetMPs.push_back(pMP);
                                }
                            }
                        }

                        cout << "[TagLC] Collected " << vpTargetMPs.size() << " unique MapPoints from historical side for fusion." << endl;

                        // 执行融合
                        if(!CorrectedPosesMap.empty() && !vpTargetMPs.empty()) {
                            // 调用与 CorrectLoop 中完全相同的 SearchAndFuse 函数
                            // 它会用 CorrectedPosesMap 中的新位姿，去投影 vpTargetMPs 中的点，并进行融合
                            SearchAndFuse(CorrectedPosesMap, vpTargetMPs);
                        }

                        // 三、本质图优化
                        cout << "[TagLC] Starting Essential Graph Optimization." << endl;
                        ...
                        for(KeyFrame* pKFi : vpLocalKFs) {
                            ...
                            for(auto vit2=vpLocalKFs.begin(); vit2!=vpLocalKFs.end(); vit2++)
                                LoopConnections[pKFi].erase(*vit2);
                        }
                        
                        KeyFrame* pLoopMatchedKF = nullptr;
                        if(!vpHistoricalKFs.empty()){
                            pLoopMatchedKF = vpHistoricalKFs.front();
                        }
                        ...
                        if(pLoopMatchedKF)
                        {
                            ...
                            if(mpTracker->mSensor==System::IMU_MONOCULAR && !pCurrentMap->GetIniertialBA2())
                                bFixedScale=false;
                            // 调用本质图优化，NonCorrectedSim3 是我们提前备份好的，CorrectedPosesMap 是融合时用的
                            Optimizer::OptimizeEssentialGraph(pCurrentMap, pLoopMatchedKF, mpCurrentKF, NonCorrectedSim3, CorrectedPosesMap, LoopConnections, bFixedScale);
                            // 添加回环边：效果存疑？？？
                            pLoopMatchedKF->AddLoopEdge(mpCurrentKF);
                            mpCurrentKF->AddLoopEdge(pLoopMatchedKF);
                        }

                    }
                    // 四、全局BA
                    // Optimizer::GlobalBundleAdjustemnt(pCurrentMap, 20, &mbStopGBA, pCurrentMap->GetMaxKFid(), false);
                    if(!pCurrentMap->isImuInitialized() || (pCurrentMap->KeyFramesInMap()<50 && mpAtlas->CountMaps()==1))
                        {
                            mbRunningGBA = true;
                            mbFinishedGBA = false;
                            mbStopGBA = false;
                            ...
                            // 新线程运行GBA的完整流程
                            mpThreadGBA = new thread(&LoopClosing::RunGlobalBundleAdjustment, this, pCurrentMap, mpCurrentKF->mnId);
                        }
                    // mpLocalMapper->Release();
                    cout << "LoopClosing::Run -> Global BA finished." << endl;
                }
                mpCurrentKF->mbForceGlobalBA = false; // 重置标记
            }
...
#endif
            if((bFindedRegion && mbActiveLC) || mbMergeDetectedzyq)
            {
                if(mbMergeDetected)
                {
                                             ...
                                             int &nNumCoincidences, std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs, CameraMatch &mCameraPair)
{
    ...
    // 部分排除共视
    // const vector<KeyFrame*> vCurrentCovisibles = mpCurrentKF->GetBestCovisibilityKeyFrames(15);
    // set<KeyFrame*> spConnectedKeyFrames(vCurrentCovisibles.begin(), vCurrentCovisibles.end());
    // spConnectedKeyFrames.insert(mpCurrentKF);
    ...
    for(KeyFrame* pKFi : vpBowCand)
    {
        ...
        // cout << "Detect A: " << numBoWMatches << "?" << nBoWMatches << endl;
...
{  
...

    {
        ...
        if(!mbStopGBA)
        {
            ...
            cout << "[RunGBA]finished!" << endl;
            mpLocalMapper->Release();
            cout << "[RunGBA]released!" << endl;
