                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
       ...
       const map<KeyFrame*,tuple<int,int,int,int>> observations = pMP->GetObservations();
        ...
        for(map<KeyFrame*,tuple<int,int,int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            // Process observation from Camera 1 (Main/Left)
            ...
            // Process observation from Camera 2 (Right)
            if(pKF->mpCamera2){
            ...
            // Process observation from Camera 3 (Side-Left)
            if(pKF->mpCamera3){
                int sideleftIndex = get<2>(mit->second);

                if(sideleftIndex != -1){
                    sideleftIndex = sideleftIndex - pKF->NLeft - pKF->NRight;

                    Eigen::Matrix<double,2,1> obs;
                    cv::KeyPoint kp = pKF->mvKeysSideLeft[sideleftIndex];
                    obs << kp.pt.x, kp.pt.y;
            ...
            // Process observation from Camera 4 (Side-Right)
            if(pKF->mpCamera4){
                int siderightIndex = get<3>(mit->second);

                if(siderightIndex != -1){
                    siderightIndex = siderightIndex - pKF->NLeft - pKF->NRight - pKF->NSideLeft;

                    Eigen::Matrix<double,2,1> obs;
                    cv::KeyPoint kp = pKF->mvKeysSideRight[siderightIndex];
                    obs << kp.pt.x, kp.pt.y;
    ...
    for(const auto pKFi : vpKFs)
    {
        ...
        for(const auto& obs_pair : observations)
        {
            ...
            // 获取观测到的位姿 T_cam_tag (测量)
            Sophus::SE3f T_cam_tag(obs.R_cam_tag.cast<float>(), obs.t_cam_tag.cast<float>());

            // 如果观测来自于非主相机，需要转换到主相机坐标系下
            if (obs.camID == 1) {        // cam2 (Right)
                T_cam_tag = pKFi->GetRelativePoseTlr() * T_cam_tag;
            } else if (obs.camID == 2) { // cam3 (Side-Left)
                T_cam_tag = pKFi->GetRelativePoseTlsl() * T_cam_tag;
            } else if (obs.camID == 3) { // cam4 (Side-Right)
                T_cam_tag = pKFi->GetRelativePoseTlsr() * T_cam_tag;
            }

            // 创建一个一元边 (Unary Edge) 来约束关键帧的位姿
            auto* e = new g2o::EdgeSE3Prior();

            // 顶点是当前关键帧的位姿
            g2o::VertexSE3Expmap* vSE3 = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(pKFi->mnId));
            if(!vSE3) continue; // 如果顶点不存在于优化器中，则跳过

            e->setVertex(0, vSE3);

            // 测量值是根据Tag计算出的相机位姿 T_c_w
            // T_c_w = T_c_tag * T_tag_w = T_c_tag * (T_w_tag)^-1
            Sophus::SE3f T_w_tag_sophus(R_w_tag.cast<float>(), t_w_tag.cast<float>());
            Sophus::SE3f T_c_w = T_cam_tag * T_w_tag_sophus.inverse();
            e->setMeasurement(Converter::toSE3Quat(T_c_w));

            // 设置信息矩阵 (权重)
            int obsCount = tagStorage.GetObservationCount(tagId);
            double weight = 10.0 * (1.0 - exp(-obsCount/5.0)); // 示例权重
            // weight = 10.0;
...
int Optimizer::PoseOptimization(Frame *pFrame, vector<Tracking::TagDetection> mCurrentFrameTags)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    // --- 在 PoseOptimization 中加入Tag约束 ---
    auto& tagStorage = TagStorage::Instance();
    if (!mCurrentFrameTags.empty())
    {
        for (const auto& obs : mCurrentFrameTags)
        {
            // 1. 获取Tag在世界坐标系下的先验位姿
            Eigen::Matrix3d R_w_tag;
            Eigen::Vector3d t_w_tag;
            // 使用 tagRead 读取已经过RANSAC等方法优化后的稳定位姿
            if (!tagStorage.tagRead(obs.id, R_w_tag, t_w_tag))
                continue; // 如果Tag位姿未知，则跳过

            // 2. 获取当前帧观测到的 T_cam_tag (测量值)
            Sophus::SE3f T_cam_tag_measured(obs.R_cam_tag.cast<float>(), obs.t_cam_tag.cast<float>());

            // 3. 如果观测来自非主相机，转换到主相机坐标系下
            // 这个逻辑与您在BA中处理多相机的逻辑完全一致
            if (obs.camID == 1) {        // cam2 (Right)
                T_cam_tag_measured = pFrame->GetRelativePoseTlr() * T_cam_tag_measured;
            } else if (obs.camID == 2) { // cam3 (Side-Left)
                T_cam_tag_measured = pFrame->GetRelativePoseTlsl() * T_cam_tag_measured;
            } else if (obs.camID == 3) { // cam4 (Side-Right)
                T_cam_tag_measured = pFrame->GetRelativePoseTlsr() * T_cam_tag_measured;
            }

            // 4. 计算期望的相机位姿 T_c_w 作为测量值
            // T_c_w = T_c_tag * T_tag_w = T_c_tag * (T_w_tag)^-1
            Sophus::SE3f T_w_tag_sophus(R_w_tag.cast<float>(), t_w_tag.cast<float>());
            Sophus::SE3f T_c_w_expected = T_cam_tag_measured * T_w_tag_sophus.inverse();

            // 5. 创建 g2o 一元边 (Prior Edge)
            auto* e = new g2o::EdgeSE3Prior();
            e->setVertex(0, optimizer.vertex(0)); // 连接到当前帧的位姿顶点
            e->setMeasurement(Converter::toSE3Quat(T_c_w_expected));

            // 6. 设置信息矩阵 (权重)
            // 在PoseOptimization中，权重可以设置得相对简单，也可以沿用BA中的策略
            int obsCount = tagStorage.GetObservationCount(obs.id);
            // 使用一个增长较快但有上限的权重函数
            double weight = 10.0 * (1.0 - exp(-obsCount/2.0)); 
            Eigen::Matrix<double,6,6> info = Eigen::Matrix<double,6,6>::Identity();
            info.block<3,3>(0,0) *= 0.5 * weight; // 旋转权重
            info.block<3,3>(3,3) *= 1.0 * weight; // 平移权重
            e->setInformation(info);

            // PoseOptimization中通常不使用鲁棒核，因为我们相信Tag的先验位姿是准确的
            // 如果需要，也可以加上
            // g2o::RobustKernelHuber* rk = new g2o::RobustKernelHuber;
            // e->setRobustKernel(rk);
            // rk->setDelta(1.0);
