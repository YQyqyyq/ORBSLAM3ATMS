    ...
    P2Fconverter()
{
    ...
    else
    {
        ...
        // mpAtlas->CreateNewMap();本来就注释了
        // mpAtlas->ChangeMap(mpAtlas->GetAllMaps()[0]);//zyqzyq
    ...
    if(isRead)
    {
        cout << "Map loaded, setting Force-Merge flag." << endl;
        mpLoopCloser->mbForceMergeAfterLoad = true;
    }
    ...
    mpLoopCloser = new LoopClosing(mpAtlas, mpKeyFrameDatabase, mpVocabulary, mSensor!=MONOCULAR, mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO || mSensor==IMU_RGBD || mSensor==IMU_MULTI); // mSensor!=MONOCULAR);
    // mMutexGlobalBA = std::mutex();
...
Sophus::SE3f System::TrackMulti(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &imSideLeft, const cv::Mat &imSideRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    for (auto& img : images) {
        img(cv::Rect(0, 0, MASK_SIZE, img.rows)).setTo(0);                    // 左
        img(cv::Rect(img.cols-MASK_SIZE, 0, MASK_SIZE, img.rows)).setTo(0);   // 右
        img(cv::Rect(0, 0, img.cols, MASK_SIZE)).setTo(0);                    // 上
        img(cv::Rect(0, img.rows-MASK_SIZE, img.cols, MASK_SIZE)).setTo(0);   // 下
    }
