...
void Tracking::Track()
{
    ...
    // cout << "地图" << pCurrentMap->GetId() << endl;
    // if(!pCurrentMap)
    // {
    //     // When the program is just started, ORB-SLAM3 will create a new map.
    //     // Here, we first check whether there is a map in the atlas.
    //     // If so, we change to the map with most keyframes.
    //     vector<Map*> vpMaps = mpAtlas->GetAllMaps();
    //     if(!vpMaps.empty())
    //     {
    //         int max_KF = 0;
    ...
    //         for(Map* pMap : vpMaps)
    //         {
    //             if(pMap->KeyFramesInMap() > max_KF)
    //             {
    //                 max_KF = pMap->KeyFramesInMap();
    //                 pBestMap = pMap;
    //             }
    //         }
    //         if(pBestMap)
    ...
    //             pCurrentMap = pBestMap;
    //             cout << "Map loaded. Change to map " << pBestMap->GetId() << endl;
    //             mbOnlyTracking = true;
    //             cout << "Only tracking mode enabled" << endl;
    //         }
    //     }
    // }
...
//     }
    ...
    else
    {
        ...
        {
            // if(!bOK)
            //     cout << "Fail before track local map!" << endl;
            if(bOK)
            {
                bOK = TrackLocalMap();
                // cout << "Track local map: " << (bOK ? "Success" : "Fail") << endl;
...
void Tracking::MultiInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        // 尝试使用Tag进行初始化位姿
        bool bPoseFromTag = false;
        std::vector<Sophus::SE3f> vValidPoses;
        if(!mCurrentFrameTags.empty())
        {   
            for (const auto& tag : mCurrentFrameTags)
            {
                ...
                // T_world_tag: Tag在世界坐标系下的位姿 (Tw_tag)
                Sophus::SE3f T_world_tag(R_w_tag.cast<float>(), t_w_tag.cast<float>());
                // T_cam_tag: Tag在相机坐标系下的位姿 (Tc_tag)
                Sophus::SE3f T_cam_tag(tag.R_cam_tag.cast<float>(), tag.t_cam_tag.cast<float>());

                // 将其他相机的观测统一到主相机(cam0)坐标系下
                Sophus::SE3f T_c0_cn = Sophus::SE3f(); // 从cam_n到cam_0的变换
                if (tag.camID == 1) {        // cam1 (Right)
                    T_c0_cn = mTlr;
                } else if (tag.camID == 2) { // cam2 (Side-Left)
                    T_c0_cn = mTlsl;
                } else if (tag.camID == 3) { // cam3 (Side-Right)
                    T_c0_cn = mTlsr;
                }
                Sophus::SE3f T_c0_tag = T_c0_cn * T_cam_tag;
                // 计算相机在世界坐标系下的位姿 T_w_c0 = T_w_tag * T_tag_c0 = T_w_tag * T_c0_tag.inverse()
                // Frame::SetPose 需要的是 Tcw, 即 T_c0_w
                Sophus::SE3f T_c0_w = T_c0_tag * T_world_tag.inverse();
                vValidPoses.push_back(T_c0_w);
            }
        }
        ...
        {
            // 对所有计算出的有效位姿进行平均
            Sophus::SE3f::Tangent avg_log = Sophus::SE3f::Tangent::Zero();
            for(const auto& pose : vValidPoses) {
                avg_log += pose.log();
            }
            avg_log /= static_cast<float>(vValidPoses.size());
            
            mCurrentFrame.SetPose(Sophus::SE3f::exp(avg_log));
            bPoseFromTag = true;
            std::cout << "[Tracking] Multi-Initialization from AprilTag pose." << std::endl;
        }
        
        if(!bPoseFromTag)
        {
            // 如果没有通过Tag成功设置位姿，则回退到原点初始化
            mCurrentFrame.SetPose(Sophus::SE3f());
            std::cout << "[Tracking] Multi-Initialization from origin." << std::endl;
        }
        // --------------------------------------
...
bool Tracking::TrackWithTag()
{
    ...
    std::vector<float> weights;             // 权重来自观测次数
    ...
    for (const auto& tag : currentTags) {
        ...
        if(tag.t_cam_tag.norm() > 3.0 || tag.t_cam_tag.norm() < 0.1)// squaredNorm
            continue;
    ...
    // Optimize frame pose with all matches
    Optimizer::PoseOptimization(&mCurrentFrame, mCurrentFrameTags);
...
void Tracking::CreateNewKeyFrame()
{
    ...
    // Tag观测写入与Tag回环标记
    bool bTagGBAForced = false;
    // 检测结果转为 TagObs 并写入 TagStorage
    for(const auto& d : mCurrentFrameTags){
        // 距离筛选 2m
        double distance = d.t_cam_tag.squaredNorm();
        if(distance > 4.0 || distance < 0.01)
            continue;
        // 视角筛选 60度
        Eigen::Vector3d tag_forward_in_tag(0, 0, 1);
        Eigen::Vector3d tag_forward_in_cam = d.R_cam_tag * tag_forward_in_tag;
        Eigen::Vector3d camera_to_tag_dir = d.t_cam_tag.normalized();
        double cos_angle = tag_forward_in_cam.dot(camera_to_tag_dir);
        ...
        // 检查这个Tag是否是首次在关键帧中出现
        if (mnLastLoopClosingByTagId.find(d.id) == mnLastLoopClosingByTagId.end()) {
            // 如果是第一次，记录下它的“锚点”关键帧ID
            mnLastLoopClosingByTagId[d.id] = pKF->mnId;
            std::cout << "[TagLC] First-time KF observation of Tag " << d.id 
                          << ". Anchoring at KF " << pKF->mnId << endl;
        }
        // 如果不是第一次出现，检查是否满足触发全局BA的条件
        else {
            if (pKF->mnId < 100)
                continue;
            if (pKF->mnId < mnLastRelocFrameId + 30)
                continue;
            if (bTagGBAForced)
                continue;

            // 同个tag触发间隔大于50，全局触发间隔大于30
            if ((pKF->mnId > mnLastLoopClosingByTagId.at(d.id) + 50) && (pKF->mnId > mnLastTagGBAKFid + 30)) {
                // 检查Tag观测可靠性
                int observationCount = TagStorage::Instance().GetObservationCount(d.id);
                if(observationCount < 5)
                    continue;
                double tagError;
                Eigen::Matrix3d R_w_tag;
                Eigen::Vector3d t_w_tag;
                if(!TagStorage::Instance().tagRead4LC(d.id, R_w_tag, t_w_tag, tagError))
                    continue;
                if (tagError < 0.3) {
                    Sophus::SE3f mTagGBAPose_cam_tag(d.R_cam_tag.cast<float>(), d.t_cam_tag.cast<float>());
                    pKF->mTagGBAPose_cam_tag = mTagGBAPose_cam_tag;
                    if (d.camID == 1) {        // cam2 (Right)
                        pKF->mTagGBAPose_cam_tag = mTlr * mTagGBAPose_cam_tag;
                    } else if (d.camID == 2) { // cam3 (Side-Left)
                        pKF->mTagGBAPose_cam_tag = mTlsl * mTagGBAPose_cam_tag;
                    } else if (d.camID == 3) { // cam4 (Side-Right)
                        pKF->mTagGBAPose_cam_tag = mTlsr * mTagGBAPose_cam_tag;
                    }
                    //
                    Sophus::SE3f T_w_tag(R_w_tag.cast<float>(), t_w_tag.cast<float>());
                    Sophus::SE3f T_w_c_correct = T_w_tag * pKF->mTagGBAPose_cam_tag.inverse();
                    Sophus::SE3f T_w_c_wrong = pKF->GetPoseInverse();
                    double pose_diff = (T_w_c_correct.translation() - T_w_c_wrong.translation()).norm();
                    if(pose_diff < 0.12) // tag的尺寸
                        continue;
                    
                    cout << "[TagLC] Tag" << d.id << " Error: " << tagError << "m; ";
                    cout << "Diff: " << pose_diff << "m." << endl;

                    pKF->mbForceGlobalBA = true;
                    pKF->mTagGBAId = d.id;
                    mnLastLoopClosingByTagId[d.id] = pKF->mnId;
                    bTagGBAForced = true;
                    cout << "[TagLC] Forcing Tag Global BA with KF：" << pKF->mnId << " due to Tag " << d.id << endl;
                    cout << "[TagLC] Tag" << d.id << "LastLC: KF" << mnLastLoopClosingByTagId.at(d.id) << endl;
                    // 给关键帧打TagBA标记
                }
            }
        }
    ...
    // 更新全局和所有相关Tag的触发记录
    if (bTagGBAForced) {
        mnLastTagGBAKFid = pKF->mnId;
        for (const auto& tag : mCurrentFrameTags) {
            mnLastLoopClosingByTagId[tag.id] = pKF->mnId;
        }
    }
        ...
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(bTcw)
            {
                ...
                if(nGood<50)
                {
                    ...
                    if(nadditional+nGood>=80)
                    {
                        nGood = Optimizer::PoseOptimization(&mCurrentFrame);
                        //cout << " PO nGood:" << nGood << endl;
                        // If many inliers but still not enough, search by projection again in a narrower window
                        // the camera has been already optimized with many points
                        if(nGood>40 && nGood<80)
                        {
                            sFound.clear();
